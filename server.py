...
def get_sentiment_score(text):
    """Tokenizes the input text and returns the predicted sentiment score."""
    inputs = tokenizer(text, return_tensors='pt', padding=True, truncation=True, max_length=512)
    with torch.no_grad():
        outputs = model(**inputs)
    logits = outputs.logits
    sentiment = torch.argmax(logits, dim=-1).item()  # Convert tensor to integer
    return sentiment
...
def predict():
    ...
    # Predict sentiment scores for each input
    teachingscore = get_sentiment_score(teaching)
    placementsscore = get_sentiment_score(placements)
    collaboration_with_companiesscore = get_sentiment_score(collaboration_with_companies)
    infrastructurescore = get_sentiment_score(infrastructure)
    hostelscore = get_sentiment_score(hostel)
    libraryscore = get_sentiment_score(library)

    teacher1score = get_sentiment_score(teacher1)
    teacher2score = get_sentiment_score(teacher2)
    teacher3score = get_sentiment_score(teacher3)
    teacher4score = get_sentiment_score(teacher4)
    teacher5score = get_sentiment_score(teacher5)
    teacher6score = get_sentiment_score(teacher6)

    # Get current time
    time = datetime.now().strftime("%m/%d/%Y (%H:%M:%S)")

    # Store feedback data in CSV files
    write_to_csv_departments(
        time, teachingscore, teaching, placementsscore, placements,
        collaboration_with_companiesscore, collaboration_with_companies,
        infrastructurescore, infrastructure, hostelscore, hostel, libraryscore, library
    )

    write_to_csv_teachers(
        teacher1, teacher1score, teacher2, teacher2score, teacher3, teacher3score,
